...
def _generate_source_id(db: Session, vendor: str, name: str, tenant_id: str) -> str:
    raw = vendor.strip() if vendor else name.strip()
    prefix = re.sub(r"[^A-Z0-9]", "", raw.upper())[:5] or "UNK"
    existing = db.query(Source).filter(Source.tenant_id == tenant_id, Source.source_id.like(f"SRC-{prefix}-%")).count()
    seq = existing + 1
...
def create_source(payload: dict[str, Any], db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    ...
    source_id = _generate_source_id(db, vendor, name, tenant_id)
...
@router.get("")
def list_sources(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    tenant_id = user.get("tenant_id", "default")
...
def get_source(source_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    tenant_id = user.get("tenant_id", "default")
...
def list_source_events(source_id: str, page: int = 1, page_size: int = 50, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    tenant_id = user.get("tenant_id", "default")
    source = db.query(Source).filter(Source.tenant_id == tenant_id, Source.source_id == source_id).first()
    if not source:
